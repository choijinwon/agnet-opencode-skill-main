...
모델 목록이 표시된 상태라면 숫자는 TODO 단계가 아니라 모델 번호 선택입니다.
...
`aiu_custom/`, `local_serving/`, `saved_model/`, `config/`, `requirements.txt`, `input_example.json` 변환은 3번 추가 시퀀스에서 `runtest_2.py` 기준으로 실행합니다.
추론 테스트는 이 자동 준비 이후 단계에서만 실행합니다.

모델 목록이 없고 TODO Guide가 표시된 상태라면 숫자는 TODO 단계 선택입니다.
