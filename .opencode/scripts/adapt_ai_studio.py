...
def build_report(project: Path, entrypoint_arg: str | None, sample: str, execute: bool, force: bool) -> AdaptReport:
    ...
    if error:
        ...
        if error == "entrypoint_not_found":
            ...
            report.next_steps.append("예: python .opencode/scripts/adapt_ai_studio.py --project <project> --entrypoint run.py")
    ...
    report.planned_changes.extend(
        [
            ...
            "create_if_missing: ai_studio/input_example.json",
    ...
    write_if_missing(project / "ai_studio" / "input_example.json", '{\n  "inputs": []\n}\n', execute, changed, skipped)
    ...
    report.next_steps.extend(
        [
            ...
            f"python .opencode/scripts/doctor.py --workspace . --project {project} --entrypoint {rel(entrypoint, project)}",
            f"python .opencode/scripts/check_environment.py --project {project} --entrypoint {rel(entrypoint, project)}",
        ]
    )
    return report
