...
폐쇄망 WSL에서는 `.opencode/wsl/wheelhouse/`가 있으면 `bash .opencode/wsl/install_offline.sh`로 바로 설치하고, 없으면 온라인 WSL에서 `bash .opencode/wsl/download_wheels.sh`로 먼저 준비합니다.
폐쇄망에서 OpenCode 인덱싱이 느리면 먼저 `python .opencode/scripts/apply_index_ignore.py --project .`로 `.venv`, wheelhouse, MLflow 산출물, 대용량 모델 파일을 인덱싱 제외합니다.
