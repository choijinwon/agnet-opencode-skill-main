...
```text
AI Studio 런치 -> 읽기 전용, model_found 확인
AI Studio 빌드 -> 생성/수정/설치/실행
...
이 파일은 AI Studio 런치 모드 진입 안내용 전역 참고 문서입니다.
현재 활성 모드가 AI Studio 빌드 모드이거나 AI Studio 빌드 탭 프롬프트가 적용된 상태에서는 AI Studio 런치 읽기 전용 규칙을 적용하지 않습니다.
AI Studio 빌드 모드에서 사용자가 `1`, `2`, `3`, `sklearn`, `pytorch`, `tensorflow` 중 하나를 입력하면 안내만 하지 말고 즉시 선택된 샘플 복사 명령을 실행합니다.
...
새 채팅 세션의 첫 assistant 응답에서는 사용자의 첫 입력 내용과 관계없이 AI Studio Guide를 먼저 출력합니다.
그 다음에는 곧바로 현재 워크스페이스를 분석해 모델 있음/없음을 확인합니다.
AI Studio 런치 모드는 읽기 전용입니다. 파일 생성, 수정, 삭제, 복사, 설치, 실행, 커밋, 푸시는 AI Studio 빌드 단계에서만 진행합니다.
...
- `아무거나`
- `분석해줘`
- `sklearn 샘플 생성해줘` 같은 AI Studio 빌드 단계 요청
- 그 밖의 구체적인 작업 요청

AI Studio Guide를 먼저 출력한 뒤에는 다음 기준으로 이어서 응답합니다.
...
- 첫 메시지가 수정/복사/실행/설치/커밋/푸시 요청이면 AI Studio 런치 모드에서는 수행하지 않고 AI Studio 빌드 단계로 안내합니다.
- 같은 채팅 세션에서는 사용자가 명시적으로 다시 요청하지 않는 한 AI Studio Guide를 반복 출력하지 않습니다.

다음 표현은 AI Studio Guide 재출력 요청으로 처리합니다.
...
- AI Studio 런치 모드에서는 파일이나 실행 상태를 변경하지 않습니다.
